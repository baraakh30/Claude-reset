...
function printInfo(state, config) {
  ...
  if (process.platform === 'linux') {
    console.log(`Journal logs    :  journalctl --user -u csk -f`);
  }
  if (fs.existsSync(PID_FILE)) {
    console.log(`Daemon PID      :  ${fs.readFileSync(PID_FILE, 'utf8').trim()}`);
  }
  const { PLIST_FILE, SYSTEMD_FILE } = require('./paths');
  if (process.platform === 'darwin' && fs.existsSync(PLIST_FILE)) {
    console.log(`Daemon type     :  launchd  (${PLIST_FILE})`);
  } else if (process.platform === 'linux' && fs.existsSync(SYSTEMD_FILE)) {
    console.log(`Daemon type     :  systemd user service  (${SYSTEMD_FILE})  ${chalk.yellow('[experimental]')}`);
  }
  console.log();
}

function printValidation(config, state) {
  const { validate } = require('./validate');
  const { warnings, infos } = validate(config, state);
  ...
  for (const w of warnings) {
    console.log(chalk.yellow('  ⚠ ') + chalk.yellow(w.message));
    if (w.suggestion) console.log(chalk.dim('    → ' + w.suggestion));
  }

  for (const info of infos) {
    console.log(chalk.cyan('  ℹ ') + chalk.dim(info.message));
  }

  if (warnings.length > 0 || infos.length > 0) console.log();
}

module.exports = { printStatus, printInfo, printValidation, formatTime, formatRelative };
